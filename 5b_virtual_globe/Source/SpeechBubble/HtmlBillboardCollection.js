/*global define*/
define([
  '../SpeechBubble/HtmlBillboard',
  '../Core/EllipsoidalOccluder',
  '../Core/DeveloperError'
], function(
    HtmlBillboard, EllipsoidalOccluder, DeveloperError) {
    'use strict';

  /**
   * @param {Cesium.Scene} scene The Cesium Scene that we're tracking.
   */
  const HtmlBillboardCollection = function (scene, container) {
    this.scene = scene;
    this.children = [];
    ...
    if (container) {
      ...
      var cswidget = document.getElementsByClassName(
        "cesium-viewer-cesiumWidgetContainer"
      )[0];
      if (cswidget) {
        cswidget.parentNode.insertBefore(
          this.childContainer,
          cswidget.nextSibling
        );
      } else {
  ...
  return HtmlBillboardCollection;
});
