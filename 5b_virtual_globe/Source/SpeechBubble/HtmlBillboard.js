...
define([
  ...
  HtmlBillboard.prototype._updateView = function (scene, camera, occluder) {
    ...
    // Check position
    newViewPos = SceneTransforms.wgs84ToWindowCoordinates(
        scene,
        this._position,
        {}
    );
    ...
    if (this.distanceDisplayCondition) {
      var dist = Cartesian3.distance(this._position, camera.positionWC);

      if (
          dist < this.distanceDisplayCondition.near ||
          dist > this.distanceDisplayCondition.far
      ) {
        this._setVisible(false);
        return;
      }
